...
# NOTE: This must come AFTER all API routes to avoid conflicts
...
def dashboard_stats():
    ...
    try:
        ...
        recent_sessions = DatabaseManager.get_recent_sessions(user_id, limit=50)

        stats['recent_sessions'] = [session.to_dict() for session in recent_sessions]

        # Debug logging
        print(f"[DEBUG] User {user_id} has {len(recent_sessions)} sessions")
...
def debug_sessions():
    ...
    try:
        all_sessions = ScrapingSession.query.order_by(ScrapingSession.started_at.desc()).limit(10).all()
        return jsonify({
            'total_count': ScrapingSession.query.count(),
            'sessions': [session.to_dict() for session in all_sessions]
        })
    except Exception as e:
        return jsonify({'error': str(e)}), 500

# Alternative endpoint for getting sessions without auth (for testing)
@app.route('/api/sessions/all')
def get_all_sessions():
    """Get all sessions - no auth required (for testing)"""
    try:
        all_sessions = ScrapingSession.query.order_by(ScrapingSession.started_at.desc()).limit(50).all()
        return jsonify({
            'sessions': [session.to_dict() for session in all_sessions],
...
if __name__ == '__main__':
    ...
    # Run with threaded mode and no reloader to prevent hanging
